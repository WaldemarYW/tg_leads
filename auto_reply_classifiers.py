...
STOP_PHRASES = [
    ...
    "без пк",
    "без пк не можу",
    "мені потрібна підробіток",
    "мне нужна подработка",
    "лише підробіток",
    "только подработка",
    "не підійде фултайм",
    "не подойдет фултайм",
    "не підійде повна зайнятість",
    "не подойдет полная занятость",
    "отпис",
    "stop",
    "unsubscribe",
    "not interested",
...
VOICE_TEXT_TRIGGER_STEPS = {"company_intro", "voice_wait", "value_hook", "work_model"}
SOFT_DOUBT_HINTS = (
    "сумніва",
    "сомнева",
    "не впевн",
...
def classify_local_intent(text: str, last_step: Optional[str] = None) -> Intent:
    ...
    if any(marker in t for marker in SOFT_DOUBT_HINTS):
        return Intent.QUESTION
    if is_stop_phrase(text):
        return Intent.STOP
